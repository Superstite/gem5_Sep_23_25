...
def run_selftests():
    ...
    def verify_value(sym_name, val):
        ...
        verify(
            sym.str_value == val,
            'expected {} to have the value "{}", had the value "{}"'.format(
                sym_name, val, sym.str_value
            ),
        )
    ...
    def assign_and_verify_value(sym_name, val, new_val):
        ...
        verify(
            sym.set_value(val),
            "assigning '{}' to {} unexpectedly failed".format(val, sym_name),
        )
        verify(
            sym.str_value == new_val,
    ...
    def assign_and_verify_user_value(sym_name, val, user_val, valid):
        ...
        verify(
            sym.set_value(val) == valid,
            "expected the user value '{}' to be {} for {}, was not".format(
                val, "valid" if valid else "invalid", sym_name
            ),
        )
        verify(
            sym.user_value == user_val,
            "the assigned user value '{}' wasn't reflected in user_value "
            "on the symbol {}. Instead, the new user_value was '{}'. The "
            "old user value was '{}'.".format(
                user_val, sym_name, sym.user_value, sym_old_user_val
            ),
        )
    ...
    verify_equal(
        _ordered_unique([1, 2, 3, 2, 1, 2, 3, 4, 3, 2, 1, 0]), [1, 2, 3, 4, 0]
    )
    ...
    def verify_eval_bad(expr):
        try:
            ...
            fail(
                'expected eval_string("{}") to throw KconfigError, '
                "didn't".format(expr)
            )
    ...
    def verify_custom_str(item, s):
        verify_equal(
            item.custom_str(lambda sc: "[{}]".format(sc.name)), s[1:-1]
        )
...
""",
    ...
    verify_str(
        c.named_choices["CHOICE"].nodes[0].next.item,
        """
choice
	tristate "no name"
	optional
""",
    ...
    verify_str(
        c.named_choices["CORRECT_PROP_LOCS_CHOICE"],
        """
...
""",
    ...
    verify_str(
        c.syms["SIMPLE_MENU_HOOK"].nodes[0].next,
        """
menu "simple menu"
""",
    )
    ...
    verify_str(
        c.syms["ADVANCED_MENU_HOOK"].nodes[0].next,
        """
menu "advanced menu"
	depends on A
	visible if B && (C || D)
...
""",
    ...
    verify_str(
        c.syms["SIMPLE_COMMENT_HOOK"].nodes[0].next,
        """
...
""",
    ...
    verify_str(
        c.syms["ADVANCED_COMMENT_HOOK"].nodes[0].next,
        """
...
""",
    ...
    def verify_deps(elms, dep_index, expected):
        verify_equal(
            " ".join(expr_str(elm[dep_index]) for elm in elms), expected
        )
...
""",
    ...
    verify_repr(
        c.syms["CHOICE_HOOK"].nodes[0].next.item,
        """
...
""",
    ...
    verify_repr(
        c.named_choices["CHOICE"].nodes[0],
        """
...
""",
    ...
    verify_repr(
        c.syms["NO_VISIBLE_IF_HOOK"].nodes[0].next,
        """
...
""",
    ...
    verify_repr(
        c.syms["VISIBLE_IF_HOOK"].nodes[0].next,
        """
...
""",
    ...
    print(
        "Testing locations, source/rsource/gsource/grsource, and "
        "Kconfig.kconfig_filenames"
    )

    def verify_locations(nodes, *expected_locs):
        verify(
            len(nodes) == len(expected_locs),
        ...
        for node, expected_loc in zip(nodes, expected_locs):
            ...
            verify(
                node_loc == expected_loc,
                "expected {} to have the location {}, had the location {}".format(
                    repr(node), expected_loc, node_loc
                ),
            )
    ...
    for srctree in "Kconfiglib", os.path.abspath("Kconfiglib"):
        ...
        verify_locations(
            c.named_choices["CHOICE_ONE_DEF"].nodes,
...
"""[
    ...
    def verify_node_path(node, *expected):
        if node.include_path != expected:
            fail(
                "Wrong include path for node {!r}. Got {}, expected {}.".format(
                    node, node.include_path, expected
                )
            )
    ...
    verify_sym_path(
        "TWO_DOWN", 0, ("Kinclude_path", 4), ("Kinclude_path_sourced_1", 4)
    )
    verify_sym_path(
        "TWO_DOWN", 1, ("Kinclude_path", 4), ("Kinclude_path_sourced_1", 9)
    )
    verify_sym_path(
        "TWO_DOWN", 2, ("Kinclude_path", 9), ("Kinclude_path_sourced_1", 4)
    )
    verify_sym_path(
        "TWO_DOWN", 3, ("Kinclude_path", 9), ("Kinclude_path_sourced_1", 9)
    )

    verify_node_path(c.top_node)
    verify_node_path(
        c.menus[0], ("Kinclude_path", 4), ("Kinclude_path_sourced_1", 4)
    )
    verify_node_path(
    ...
    def verify_prompts(items, *expected_prompts):
        verify(
            len(items) == len(expected_prompts),
        ...
        for item, expected_prompt in zip(items, expected_prompts):
            ...
            verify(
                item.prompt[0] == expected_prompt,
                "Wrong prompt for {}, expected '{}'".format(
                    repr(item), expected_prompt
                ),
            )
    ...
    verify_equal(
        expr_str(c.named_choices["DEP_CHOICE"].direct_dep), "A || B || C"
    )
    ...
    def verify_expr_items(expr, *sym_names):
        verify_equal(
            tuple(sorted(item.name for item in expr_items(expr))), sym_names
        )

    verify_expr_items(
        c.syms["TEST"].defaults[0][0], "A", "B", "C", "D", "E", "F", "G", "H"
    )

    verify_expr_items(c.syms["TEST_CHOICE"].nodes[0].prompt[1], "A", "CHOICE")
    ...
    def verify_deps(item, *dep_names):
        verify_equal(
            tuple(sorted(item.name for item in item.referenced)), dep_names
        )
    ...
    def verify_split(to_split, op, operand_strs):
        ...
        verify(
            len(operands) == len(operand_strs),
            "Wrong number of operands when {} was split by {}".format(
                to_split, "OR" if op == OR else "AND"
            ),
        )
    ...
    def verify_visibility(item, no_module_vis, module_vis):
        ...
        verify(
            ...
            "visibility {}".format(repr(item), no_module_vis, item.visibility),
        )

        c.modules.set_value(2)
        verify(
            item.visibility == module_vis,
            "expected {} to have visibility {} with modules, had "
            "visibility {}".format(repr(item), module_vis, item.visibility),
        )
    ...
    def verify_assignable_imp(item, assignable_no_modules, assignable_modules):
        ...
        for modules_val, assignable in (0, assignable_no_modules), (
            ...
            module_msg = (
                "without modules" if modules_val == 0 else "with modules"
            )
            ...
            verify(
                ...
                "was {}.".format(
                    item.name, module_msg, assignable, item.assignable
                ),
            )
    ...
    verify(
        c.syms["E"].nodes[0].parent.item == MENU, "E's parent should be a menu"
    )
    ...
    for sym_name in "HEX_NO_RANGE", "INT_NO_RANGE", "HEX_40", "INT_40":
        sym = c.syms[sym_name]
        verify(not sym.ranges, "{} should not have ranges".format(sym_name))
    ...
    verify(c.mainmenu_text == "---bar baz---", "Wrong mainmenu text")
    ...
    syms = [
        c.syms[name] for name in ("BOOL", "TRISTATE", "STRING", "INT", "HEX")
    ]
    ...
    for s in syms:
        s.unset_value()
        verify(
            s.user_value is None,
            "{} should not have a user value after being reset".format(s.name),
        )
    ...
    for not_menuconfig in (
        ...
        verify(
            not not_menuconfig.is_menuconfig,
    ...
    for name in "A", "B", "C", "D", "BOOL", "TRISTATE", "STRING", "INT", "HEX":
        verify(c.syms[name].nodes, "{} should be defined".format(name))
    ...
    for name in (
        ...
        sym = c.syms[name]
        verify(not c.syms[name].nodes, "{} should not be defined".format(name))

    print("Testing Symbol.choice")

    for name in "A", "B", "C", "D":
        verify(
            c.syms[name].choice is not None,
    ...
    verify(
        not c.syms["NOT_ALLNOCONFIG_Y"].is_allnoconfig_y,
        "NOT_ALLNOCONFIG_Y should not be allnoconfig_y",
    )
    verify(
        c.syms["ALLNOCONFIG_Y"].is_allnoconfig_y,
...
""",
    ...
    c.write_min_config(
        config_test_file, header="min. config header from param\n"
    )
    verify_file_contents(
        config_test_file,
        """\
...
""",
    ...
    for c in Kconfig("Kconfiglib/tests/Kmisc", warn=False), Kconfig(
        "Kconfiglib/tests/Kmisc", warn=False
    ):
        for item in (
            c.syms["BOOL"],
            c.syms["BOOL"].nodes[0],
    ...
    def select_and_verify(sym):
        ...
        verify(
            sym.choice.selection is sym,
            sym.name + " should be the selected symbol",
        )

        verify(
            choice.user_selection is sym,
            sym.name + " should be the user selection of the choice",
        )

        verify(
            sym.tri_value == 2, sym.name + " should have value y when selected"
        )
    ...
    def verify_mode(choice_name, no_modules_mode, modules_mode):
        ...
        verify(
            ...
            "Wrong mode for choice {} with no modules. Expected {}, got {}.".format(
                choice.name, no_modules_mode, choice.tri_value
            ),
        )

        c.modules.set_value(2)
        verify(
            choice.tri_value == modules_mode,
            "Wrong mode for choice {} with modules. Expected {}, got {}.".format(
                choice.name, modules_mode, choice.tri_value
            ),
        )
    ...
    verify(
        choice.selection is c.syms["OPT_2"],
        "Wrong choice default with TRISTATE_SYM = y",
    )

    c.syms["OPT_1"].set_value(2)
    verify(
        choice.selection is c.syms["OPT_1"],
    ...
    verify(
        c.named_choices["TRISTATE"].tri_value == 1,
    ...
    for name in "MMT_1", "MMT_2", "MMT_4", "MMT_5":
        verify(
            c.syms[name].orig_type == BOOL,
            "Expected {} to get type bool".format(name),
        )

    verify(
        c.syms["MMT_3"].orig_type == TRISTATE,
    ...
    verify(
        default_with_dep_choice.selection is c.syms["B"],
    ...
    verify(
        default_with_dep_choice.selection is c.syms["A"],
    ...
    def verify_is_weird_choice_symbol(name):
        ...
        verify(
            ...
            "{} should be a weird (non-)choice symbol".format(sym.name),
        )
    ...
    verify(len(nodes) == 10, "Wrong number of nodes after 'if' removal")
    ...
    def verify_props(desc, props, prop_names):
        ...
        verify(
            actual == expected,
            "Wrong {} properties, expected '{}', got '{}'".format(
    ...
    verify_props(
        "default",
        c.syms["MULTIDEF"].defaults,
        "A B C D E F G H I J K L M N O P Q R",
    )

    verify_props(
        "select", c.syms["MULTIDEF"].selects, "AA BB CC DD EE FF GG HH II JJ"
    )

    verify_props(
        "imply", c.syms["MULTIDEF"].selects, "AA BB CC DD EE FF GG HH II JJ"
    )
    ...
    for i in range(11):
        ...
        try:
            ...
            if "Dependency loop" not in str(e):
                fail(
                    "dependency loop in {} raised wrong KconfigError".format(
                        filename
                    )
                )
        except:
            fail(
                "dependency loop in {} raised wrong exception".format(filename)
            )
        else:
...
"""[
    ...
    def verify_variable(name, unexp_value, exp_value, recursive, *args):
        ...
        if not args:
            verify(
                var.expanded_value == exp_value,
                "expected expanded_value for {} to be '{}', was '{}'".format(
                    name, exp_value, var.expanded_value
                ),
            )

        verify(
            var.expanded_value_w_args(*args) == exp_value,
            "expected expanded_value_w_args() for '{}' to be '{}', was '{}'".format(
                name, exp_value, var.expanded_value_w_args(*args)
            ),
        )

        verify(
            var.is_recursive == recursive,
            "{} was {}, shouldn't be".format(
                name, "recursive" if var.is_recursive else "simple"
            ),
        )
    ...
    verify_variable("quote", '"$(1)" "$(2)"', '"one" ""', True, "one")
    verify_variable(
        "quote", '"$(1)" "$(2)"', '"one" "two"', True, "one", "two"
    )
    verify_variable(
        "quote", '"$(1)" "$(2)"', '"one" "two"', True, "one", "two", "three"
    )
...
""",
    ...
    def verify_recursive(name):
        try:
            ...
            fail(
                "Expected '{}' expansion to flag recursive expansion, didn't".format(
                    name
                )
            )
    ...
    verify_variable(
        "safe-fn-rec-res", "$(safe-fn-rec,safe-fn-rec-2)", "foo", True
    )
    ...
    verify_equal(
        c.env_vars, set(("ENV_1", "ENV_2", "ENV_3", "ENV_4", "ENV_5", "ENV_6"))
    )
    ...
    verify_variable(
        "location-1",
        "Kconfiglib/tests/Kuserfunctions:13",
        "Kconfiglib/tests/Kuserfunctions:13",
        False,
    )
    verify_variable(
        "location-2",
        "Kconfiglib/tests/Kuserfunctions:14",
        "Kconfiglib/tests/Kuserfunctions:14",
        False,
    )
    ...
    def verify_bad_argno(name):
        try:
            ...
            fail(
                "Expected '{}' expansion to flag wrong number of arguments, "
                "didn't".format(name)
            )
...
def test_allnoconfig(arch, srcarch):
    ...
    """
    shell(
        "make scriptconfig SCRIPT=Kconfiglib/allnoconfig.py "
...
def test_allnoconfig_walk(arch, srcarch):
    ...
    """
    shell(
        "make scriptconfig SCRIPT=Kconfiglib/examples/allnoconfig_walk.py "
        "PYTHONCMD='{}'".format(sys.executable)
    )
    shell("mv .config ._config")
    shell("scripts/kconfig/conf --allnoconfig Kconfig")
...
def test_allmodconfig(arch, srcarch):
    ...
    """
    shell(
        "make scriptconfig SCRIPT=Kconfiglib/allmodconfig.py "
        "PYTHONCMD='{}'".format(sys.executable)
    )
    shell("mv .config ._config")
...
def test_allyesconfig(arch, srcarch):
    ...
    """
    shell(
        "make scriptconfig SCRIPT=Kconfiglib/allyesconfig.py "
        "PYTHONCMD='{}'".format(sys.executable)
    )
    shell("mv .config ._config")
...
def test_sanity(arch, srcarch):
    ...
    for sym in kconf.defined_syms:
        verify(
            sym._visited == 2,
            "{} has broken dependency loop detection (_visited = {})".format(
                sym.name, sym._visited
            ),
        )
    ...
    for key, sym in kconf.syms.items():
        ...
        for dep in sym._dependents:
            verify(
                not dep.is_constant,
                "the constant symbol {} depends on {}".format(
                    dep.name, sym.name
                ),
            )
    ...
    for sym in kconf.defined_syms:
        ...
        verify(
            not (sym.orig_type not in (BOOL, TRISTATE) and sym.choice),
    ...
    for key, sym in kconf.const_syms.items():
        ...
        verify(
            sym.is_constant,
            '"{}" is in const_syms but not marked constant'.format(sym.name),
        )
        ...
        verify(
            not sym._dependents,
            '"{}" is constant but is a dependency of some symbol'.format(
                sym.name
            ),
        )
    ...
    for choice in kconf.choices:
        for sym in choice.syms:
            verify(
                sym.choice is choice,
                "{0} is in choice.syms but 'sym.choice' is not the choice".format(
                    sym.name
                ),
            )

            verify(
                sym.type in (BOOL, TRISTATE),
                "{} is a choice symbol but is not a bool/tristate".format(
                    sym.name
                ),
            )
...
def test_alldefconfig(arch, srcarch):
    ...
    """
    shell(
        "make scriptconfig SCRIPT=Kconfiglib/alldefconfig.py "
        "PYTHONCMD='{}'".format(sys.executable)
    )
    shell("mv .config ._config")
...
def test_defconfig(arch, srcarch):
    ...
    for defconfig in defconfigs:
        ...
        shell(
            "scripts/kconfig/conf --defconfig='{}' Kconfig".format(defconfig)
        )
...
def equal_configs():
    ...
    try:
        ...
        if e.errno != errno.ENOENT:
            raise
        print(
            "._config not found. Did you forget to apply the Makefile patch?"
        )
        return False
    else:
        with f:
