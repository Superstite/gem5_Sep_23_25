...
def _fix_treeview_issues():
    ...
    _treeview_rowheight = (
        font.Font(font=style.lookup("Treeview", "font")).metrics("linespace")
        + 2
    )
...
def _create_top_widgets():
    ...
    ttk.Button(
        topframe, text="Save minimal (advanced)...", command=_save_minimal
    ...
    ttk.Checkbutton(
        ...
        column=0, row=1, sticky="nsew", padx=".05c", pady="0 .05c", ipady=".2c"
    )
    ...
    _backbutton.grid(
        column=0, row=4, sticky="nsew", padx=".05c", pady="0 .05c"
    )
...
def _create_kconfig_tree(parent):
    ...
    tree = ttk.Treeview(
        frame, selectmode="browse", height=20, columns=("name",)
    )
...
def _save_minimal():
    ...
    while True:
        ...
        if _try_save(
            _kconf.write_min_config, filename, "minimal configuration"
        ):
...
def _loc_ref_item():
    ...
    return (
        _tree.identify_row(_tree.winfo_height() // 2)
        or _tree.get_children()[0]
    )
...
def _jump_to_dialog(_=None):
    ...
    def jump_to_selected(event=None):
        ...
        if event and (
            tree.identify_element(event.x, event.y) == "image"
            or _in_heading(event)
        ):
            return
    ...
    dialog.geometry(
        "+{}+{}".format(_root.winfo_rootx() + 50, _root.winfo_rooty() + 50)
    )
...
def _sorted_sc_nodes(cached_nodes=[]):
    ...
    if not cached_nodes:
        # Add symbol nodes
        for sym in sorted(
            _kconf.unique_defined_syms, key=lambda sym: sym.name
        ):
        ...
        choices = sorted(
            _kconf.unique_choices, key=lambda choice: choice.name or ""
        )
...
def _direct_dep_info(sc):
    ...
    return (
        ...
        if sc.direct_dep is _kconf.y
            ...
            TRI_TO_STR[expr_value(sc.direct_dep)],
...
def _split_expr_info(expr, indent):
    ...
    for i, term in enumerate(split_expr(expr, split_op)):
        s += "{}{} {}".format(
            indent * " ", "  " if i == 0 else op_str, _expr_str(term)
        )
...
def _select_imply_info(sym):
    ...
    if sym.rev_dep is not _kconf.n:
        s += sis(
            sym.rev_dep, 2, "Symbols currently y-selecting this symbol:\n"
        )
        s += sis(
            sym.rev_dep, 1, "Symbols currently m-selecting this symbol:\n"
        )
        s += sis(
            sym.rev_dep,
            0,
    ...
    if sym.weak_rev_dep is not _kconf.n:
        s += sis(
            sym.weak_rev_dep, 2, "Symbols currently y-implying this symbol:\n"
        )
        s += sis(
            sym.weak_rev_dep, 1, "Symbols currently m-implying this symbol:\n"
        )
        s += sis(
            sym.weak_rev_dep,
            0,
    ...
    while node.parent is not _kconf.top_node:
        ...
        path = (
            ...
            + (
                node.prompt[0]
                if node.prompt
                else standard_sc_expr_str(node.item)
            )
            + path
        )
