...
class BasicInterpreter:
        ...
        if etype == "NUM":
            ...
            if not dim1 and not dim2:
                if var in self.vars:
                    ...
                    print(
                        "UNDEFINED VARIABLE %s AT LINE %s"
                ...
                if var in self.functions:
                    ...
                    if var in self.lists:
                        ...
                        if dim1val < 1 or dim1val > len(self.lists[var]):
                            print(
                                "LIST INDEX OUT OF BOUNDS AT LINE %s"
                                % self.stat[self.pc]
                            )
                            raise RuntimeError
            ...
            if dim1 and dim2:
                if var in self.tables:
                    ...
                    if (
                        dim1val < 1
                        or dim1val > len(self.tables[var])
                        or dim2val < 1
                        or dim2val > len(self.tables[var][0])
                    ):
                        print(
                            "TABLE INDEX OUT OUT BOUNDS AT LINE %s"
                            % self.stat[self.pc]
                        )
                        raise RuntimeError
                    return self.tables[var][dim1val - 1][dim2val - 1]
            print(
                "UNDEFINED VARIABLE %s AT LINE %s" % (var, self.stat[self.pc])
            )
            raise RuntimeError
    ...
    def goto(self, linenum):
        if not linenum in self.prog:
            print(
                "UNDEFINED LINE NUMBER %d AT LINE %d"
    ...
    def expr_str(self, expr):
        ...
        if etype == "NUM":
            ...
            return "%s %s %s" % (
                self.expr_str(expr[2]),
                expr[1],
                self.expr_str(expr[3]),
            )
        elif etype == "VAR":
